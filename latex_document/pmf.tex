...
\section{Experimental Analysis: Subspace Dynamics in Complete vs.\ Incomplete Data}

We conducted computational experiments to investigate the behavior of optimization algorithms under two scenarios: complete data (standard ALS) and incomplete data (soft-impute). The results reveal a fundamental difference in subspace dynamics that has important implications for global optimization in matrix factorization.

\subsection*{Experimental Design}

Consider a synthetic matrix $Y \in \mathbb{R}^{10 \times 8}$ with known singular value structure and diagonal precision matrix $\Lambda = \mathrm{diag}(1, 2, 3)$. We compare four initialization strategies:
\begin{itemize}[itemsep=1pt]
\item[(A)] Converged solution from top subspace (global minimum)  
\item[(B)] Randomly scaled vectors from top subspace
\item[(C)] Converged solution from alternative subspace (saddle point)
\item[(D)] Randomly scaled vectors from alternative subspace
\end{itemize}

\paragraph{Part 1: Complete Data Analysis.}
Using standard ALS on complete matrix $Y$, we observed perfect \emph{subspace invariance}: initializations within the top subspace (A, B) converged to the global minimum, while initializations within the alternative subspace (C, D) converged to a saddle point. Crucially, no algorithm escaped its initialization subspace, confirming the theoretical predictions of Proposition~\ref{prop:als_subspace_invariance}.

\paragraph{Part 2: Incomplete Data Analysis.} 
We generated a masked version $\tilde{Y}$ with $20\%$ missing entries and applied the soft-impute algorithm (iterative imputation + soft-thresholded SVD) from the same four starting points.

\subsection*{Key Experimental Findings}

\paragraph{Subspace Invariance Breaking.}
In stark contrast to the complete data case, \emph{all four initializations converged to the identical solution}, regardless of their starting subspace. This represents a fundamental departure from the subspace trapping observed in Part 1.

\paragraph{Global Convergence.}
All algorithms achieved the same final loss value, suggesting convergence to a global optimum. The loss reductions varied dramatically depending on initialization quality:
\begin{itemize}[itemsep=1pt]
\item Good initializations (A): modest improvement
\item Poor initializations (D): substantial improvement ($> 30$ point reduction)
\end{itemize}

\paragraph{Algorithmic Stationarity.}
Restarting the soft-impute algorithm from converged solutions produced negligible changes ($\mathcal{O}(10^{-17})$), confirming true convergence to stationary points.

\subsection*{Theoretical Interpretation}

The experimental results reveal that \emph{missing data breaks subspace constraints} through the imputation mechanism. In the soft-impute iteration:
\[
\tilde{Y}^{(t)} \leftarrow M \odot Y + (1-M) \odot (U^{(t)} V^{(t)\top}),
\]
the imputed matrix $\tilde{Y}^{(t)}$ becomes a hybrid object combining observed data with the current factorization estimate. This hybridization has two critical effects:

\begin{enumerate}[itemsep=2pt]
\item \textbf{Information Mixing:} Missing entries are filled with linear combinations from the current estimate, effectively mixing information across different singular directions.

\item \textbf{Subspace Mobility:} The SVD of the hybrid matrix $\tilde{Y}^{(t)}$ can access the full singular spectrum of $Y$, not just the subspace determined by initialization.
\end{enumerate}

\subsection*{Implications}

This analysis demonstrates a counterintuitive phenomenon: \emph{incomplete observations can facilitate global optimization}. While missing data is typically viewed as an impediment, our experiments show that the imputation process provides implicit regularization that helps algorithms escape poor local optima.

The mechanism suggests that missing data acts as a form of \emph{stochastic perturbation} that breaks the rigid geometric constraints present in complete data scenarios. This finding has practical implications for matrix completion algorithms and suggests that moderate amounts of missing data might actually be beneficial for achieving better optimization outcomes.

The contrast between Part 1 (subspace trapping) and Part 2 (global convergence) illustrates how algorithmic behavior can change qualitatively depending on the observation pattern, highlighting the importance of considering data structure in optimization algorithm design.
